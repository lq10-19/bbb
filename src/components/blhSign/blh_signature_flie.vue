...
export default {
    ...
    mounted() {
		this.getBase64();
		document.getElementsByTagName("meta")[name='viewport'].setAttribute("content",'width=device-width');
	  },
	destroyed(){
		document.getElementsByTagName("meta")[name='viewport'].setAttribute("content","width=device-width,minimum-scale=1.0,maximum-scale=1.0,user-scalable=no");
	},
  	methods:{
	 	getBase64(){
      Indicator.open({
      ...
      let url = "fileSignConfig/getPdfBase64ByPath?filePath="+this.$route.query.filePDF;
