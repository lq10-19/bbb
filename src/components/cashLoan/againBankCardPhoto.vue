...
export default {
  ...
  methods: {
    ...
    //   let url = `ocrController/queryBankCardOcrResultLog?certId=${
    //     this.customerCertId
    //   }&contractNo=${this.serialNoList[0]}&customerName=${this.customerName}`;
    ...
    upload(e, res) {
      this.type = res;
      console.log("图：", e.srcElement.files[0].type);
      if (
    ...
    //活体识别数据
    async livingBody() {
      const returnUrl = `${this.url}againConfirmSubmit`;
      let serialNoList = JSON.stringify(this.serialNoList);
      const params = {
        ...
        url: `${returnUrl}?customerId=${this.param.customerId}&serialNoList=${serialNoList}`,
