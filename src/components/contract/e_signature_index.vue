...
export default {
  ...
  created() {
    ...
    if (sessionStorage.getItem("identifyingVal")) {
      sessionStorage.removeItem("identifyingVal");
    }
  },
  methods: {
    aa() {
    ...
    submit() {
      ...
      this.$request
        .get(
          "electronAutographCtr/queryByParams?putoutNO=" +
            putoutNO +
            "&idCard=" +
            idCard +
            "&verifiCode=" +
        ...
        .then(res => {
          ...
          if (res && res.code == "200") {
            ...
            this.$router.push({
              path: "AddID",
              query: {
                infoID: "tgNIKMpTe8ifnGyS1f5E",
                // inputuserId: "1000002132752365",
                customername: resData.customername,
                contractNo: resData.contractNo,
                idCard: resData.certid
              }
            });
          }
        })
